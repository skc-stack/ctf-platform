...
<section class="ctf-dash">
    ...
    <div class="task-status-panel" id="statusPanel" style="margin-bottom:1.5rem;padding:16px;border:1px solid var(--grid);background:#161b22">
        ...
        <div class="task-progress-steps" style="display:flex;flex-direction:column;gap:8px">
            ...
            <div class="step <?= in_array($taskStatus, ['token_validated','challenge_started','completed']) ? 'done' : '' ?> <?= $taskStatus === 'challenge_started' ? 'blink' : '' ?>" id="step3">
                <span class="step-icon"><i class="bi bi-3-circle-fill"></i></span>
                <span class="step-text">解題進行中....</span>
            </div>
            <div class="step <?= $taskStatus === 'completed' ? 'done' : '' ?>" id="step4">
                <span class="step-icon"><i class="bi bi-4-circle-fill"></i></span>
                <span class="step-text">恭喜你!解題成功!</span>
            </div>
        </div>
    </div>
    ...
    .task-progress-steps .step.active.blink, .task-progress-steps .step.done.blink { animation: blink-animation 1s ease-in-out infinite }
    .task-progress-steps .step.active.blink span, .task-progress-steps .step.done.blink span { color:#ffe066;text-shadow:0 0 8px #ffe066,0 0 12px #ffaa00 }
    @keyframes blink-animation {
        0%, 100% { background:rgba(255,220,50,0.2); }
        50% { background:rgba(255,180,0,0.35); }
    }
