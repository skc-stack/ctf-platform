...
final class SubmissionService
{
    ...
    private function verifyAndAward(
        ...
        // Count previous wrong submissions to determine attempt number
        $previousWrongAttempts = $this->submissions->countWrong($studentId, $challengeId);
        $attempts = $previousWrongAttempts + 1; // +1 because this current submission is also an attempt

        $basePoints = (int)$challenge['points'];
        $solveResult = $this->solves->tryCreate($studentId, $challengeId, $taskId, $basePoints, $attempts);
        ...
        return [
            'ok' => true,
            'reason' => 'correct',
            'points' => $solveResult['points'], // actual points with decay applied
            'attempts' => $attempts,
            'new_solve' => true,
        ];
    }

    /** @return array{ok: bool, reason: string, points: int, total_score: int, new_solve: bool, attempts: int} */
