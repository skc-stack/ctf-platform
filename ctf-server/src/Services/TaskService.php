...
final class TaskService
{
    ...
    public function start(int $studentId, int $challengeId, ?int $ttlMinutes = null, ?Request $req = null): array
    {
        ...
        // Use challenge-specific time limit if set, otherwise use system default
        $challengeTimeLimit = isset($challenge['time_limit_minutes']) ? (int)$challenge['time_limit_minutes'] : null;
        $ttl = $ttlMinutes ?? $challengeTimeLimit ?? (int)Config::get('DEFAULT_TASK_TTL', 120);
