...
final class ChallengeRepository
{
    ...
    /**
     * List challenges visible to a student with pagination and optional filters.
     * @return array{rows: array<int,array<string,mixed>>, total: int, page: int, perPage: int, pages: int}
     */
    public function listForStudentPaginated(int $studentId, int $page = 1, int $perPage = 20, ?string $category = null, ?string $difficulty = null): array
    {
        $offset = max(0, ($page - 1) * $perPage);

        // Base conditions for count (without :sid since it's only used in SELECT subquery)
        $cond = ['c.status = :pub'];
        $countParams = [':pub' => self::STATUS_PUBLISHED, ':sid2' => $studentId];
        ...
        if ($difficulty !== null && $difficulty !== '') {
            $cond[] = 'c.difficulty = :diff';
            $countParams[':diff'] = $difficulty;
        }

        // Visibility: public challenge OR (has group binding AND student is in that group)
        $cond[] = '(
            NOT EXISTS (SELECT 1 FROM challenge_groups WHERE challenge_id = c.id)
            OR EXISTS (
                SELECT 1 FROM challenge_groups cg
                JOIN group_members gm ON gm.group_id = cg.group_id
                WHERE cg.challenge_id = c.id AND gm.student_id = :sid2 AND gm.status = \'active\'
            )
        )';
        ...
        // Total count
        $totalRow = Connection::fetchOne(
            "SELECT COUNT(*) AS c FROM challenges c WHERE {$whereSql}",
            $countParams
        );
        $total = (int)($totalRow['c'] ?? 0);

        // Rows with solve info (includes :sid for the solves subquery)
        $rowsParams = array_merge($countParams, [':sid' => $studentId, ':limit' => $perPage, ':offset' => $offset]);
    ...
    /**
     * Get all distinct categories for published challenges.
     * @return array<string>
     */
    public function listCategories(): array
    {
        $rows = Connection::fetchAll(
            'SELECT DISTINCT category FROM challenges WHERE status = :pub ORDER BY category',
    ...
    /**
     * Get all distinct difficulties for published challenges.
     * @return array<string>
     */
    public function listDifficulties(): array
    {
        $rows = Connection::fetchAll(
            'SELECT DISTINCT difficulty FROM challenges WHERE status = :pub ORDER BY difficulty',
            [':pub' => self::STATUS_PUBLISHED]
        );
        return array_column($rows, 'difficulty');
    }
    ...
    public function create(array $fields): int
    {
        ...
        Connection::run(
            'INSERT INTO challenges
                (uuid, teacher_id, title, slug, category, difficulty, description, points, time_limit_minutes, version, status)
             VALUES
                (:u, :t, :title, :slug, :cat, :diff, :desc, :points, :tl, :ver, :st)',
            [
                ':u'     => $uuid,
                ...
                ':tl'    => isset($fields['time_limit_minutes']) && $fields['time_limit_minutes'] !== null ? (int)$fields['time_limit_minutes'] : null,
     ...
     * @param array<string,mixed> $fields Optional: title, description, category, difficulty, points, version, time_limit_minutes.
     * @return bool True if updated.
     */
    public function update(int $id, array $fields): bool
    {
        $allowed = ['title', 'description', 'category', 'difficulty', 'points', 'version', 'time_limit_minutes'];
        $updates = [];
        $params = [':id' => $id];
