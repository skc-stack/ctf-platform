...
def create_app(config: Optional[Config] = None,
    ...
    def task():
        ...
        try:
            ...
            # Use the slug from entrypoint (e.g. "/challenge/{slug}/")
    ...
    def getflag():
        ...
        Forwards to Server to get the dynamic flag for this task."""
        c = _require_cred()
        body = request.get_json(silent=True) or {}
        task_id = body.get("task_id")
        challenge_slug = (body.get("challenge_slug") or "").strip()
        if not task_id or not challenge_slug:
            return jsonify({"error": "task_id and challenge_slug required"}), 400
        try:
            resp = ServerAPI(cfg, c).getflag(int(task_id), challenge_slug)
            data = resp.body.get("data", {})
            return jsonify({"ok": True, "flag": data.get("flag", "")})
        except ServerAPIError as e:
            return jsonify({"error": e.message}), e.status_code
